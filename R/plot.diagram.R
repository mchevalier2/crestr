...
plot_diagram <- function(x, bars=FALSE,
  ...
  if (methods::is(x)[1] == 'crestObj') {
    ...
    #}
    x <- cbind(x=x$inputs$x, x$inputs$x)
  }
  x <- x[order(x[, 1]), ]
  ...
  if(bars) {
    ...
    for(i in 2:ncol(x)) {
      for(j in 1:nrow(x)) {
        graphics::rect(x[j,1]-bar_width, cs[i], x[j,1]+bar_width, cs[i]+abs(x[j,i]), col=ifelse(x[j,i] > 0, col_pos, col_neg), lwd=0.1)
      }
      ...
      graphics::text(xlim[1] - 0.15*dX, (cs[i]+cs[i+1])/2, colnames(x)[i], cex=0.7, adj=c(1,0.5))
    }
  } else {
    for(i in 2:ncol(x)) {
  ...
  graphics::text(mean(xlim), -0.5*dY, colnames(x)[1], cex=0.8, adj=c(0.5,0))
